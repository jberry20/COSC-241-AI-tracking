...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        stop = self.numParticles
        posInLegalPos = 0
        listOfParticles = []
        while stop != 0:
            if posInLegalPos >= len(self.legalPositions):
                posInLegalPos = 0
            listOfParticles.append(self.legalPositions[posInLegalPos])
            stop -= 1
            posInLegalPos += 1
    ...
    def observe(self, observation, gameState):
        ...
        self.listOfParticles = util.nSample(distribution, values, self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.listOfParticles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            self.Dist[p] = newPosDist
            posDis = self.Dist[p]
            newPos = util.sample(posDis)
            finalPs.append(newPos)
        self.listOfParticles = finalPs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        self.particles = []
        randomPositions = self.legalPositions
        random.shuffle(randomPositions)

        while len(self.particles) < self.numParticles:
            for positions in itertools.product(randomPositions, repeat = self.numGhosts):
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] == None:
                    for j, particle in enumerate(self.particles):
                        self.particle[j] = self.getParticleWithGhostInJail(particle, ghost)
        else:
